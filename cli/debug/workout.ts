...
=== Workout Details ===`));
        ...
        if (w.source === 'intervals') {
            ...
            checks.push(
                { label: 'Avg Watts', db: w.averageWatts, raw: raw.icu_average_watts ?? raw.average_watts, tolerance: 1, unit: 'W' },
                { label: 'Max Watts', db: w.maxWatts, raw: raw.max_watts, tolerance: 1, unit: 'W' },
                { label: 'Norm Power', db: w.normalizedPower, raw: raw.normalized_power, tolerance: 1, unit: 'W' },
                { label: 'Avg HR', db: w.averageHr, raw: Math.round(raw.average_heartrate || 0), tolerance: 1, unit: 'bpm' },
                { label: 'Max HR', db: w.maxHr, raw: Math.round(raw.max_heartrate || 0), tolerance: 1, unit: 'bpm' },
                { label: 'Duration', db: w.durationSec, raw: raw.moving_time || raw.elapsed_time || raw.duration, tolerance: 1, unit: 's' },
                ...
                { label: 'Intensity', db: w.intensity, raw: raw.icu_intensity || raw.intensity, tolerance: 0.01 },
                { label: 'Kjoules', db: w.kilojoules, raw: raw.icu_joules, tolerance: 5 },
                { label: 'Cadence', db: w.averageCadence, raw: Math.round(raw.average_cadence || 0), tolerance: 1, unit: 'rpm' },
                { label: 'Decoupling', db: w.decoupling, raw: raw.decoupling, tolerance: 0.1, unit: '%' },
            ...
            checks.push(
                { label: 'Avg Watts', db: w.averageWatts, raw: raw.average_watts, tolerance: 1, unit: 'W' },
                { label: 'Max Watts', db: w.maxWatts, raw: raw.max_watts, tolerance: 1, unit: 'W' },
                { label: 'Avg HR', db: w.averageHr, raw: raw.average_heartrate, tolerance: 1, unit: 'bpm' },
                ...
                { label: 'Distance', db: w.distanceMeters, raw: raw.distance, tolerance: 1, unit: 'm' },
                { label: 'Cadence', db: w.averageCadence, raw: raw.average_cadence, tolerance: 1, unit: 'rpm' }
            );
        } else {
...
=== Field Comparison (DB vs Raw) ===`));
        ...
        checks.forEach(c => {
            ...
            if (c.raw !== undefined && c.raw !== null) {
                ...
                else if (c.db != c.raw) { // loose equality to handle null vs undefined if not caught
                     status = chalk.red('❌ MISMATCH');
                     isMismatch = true;
                }
            } else {
            ...
            if (isMismatch || options.verbose || workoutDiscrepancies > 0) {
                 ...
                 const dbDisplay = typeof dbVal === 'number' ? chalk.yellow(dbVal) : chalk.yellow(String(dbVal));
                 const rawDisplay = typeof rawVal === 'number' ? chalk.blue(rawVal) : chalk.blue(String(rawVal));
                 
                 console.log(`${c.label.padEnd(12)} | DB: ${dbDisplay}${unit} | Raw: ${rawDisplay}${unit} | ${status}`);
            }
        });
