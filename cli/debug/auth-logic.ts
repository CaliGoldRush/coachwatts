...
const authLogicCommand = new Command('auth-logic');

authLogicCommand
    .description('Debug the authentication logic')
    .action(async () => {
        console.log(chalk.green('--- Debug Auth Logic ---'));
        ...
        if (process.env.AUTH_BYPASS_USER) {
            ...
            try {
                ...
                if (user) {
                    console.log('✅ Bypass user exists:', user.id, user.email, 'isAdmin:', user.isAdmin);
                } else {
...
export default authLogicCommand;
