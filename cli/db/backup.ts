...
backupCommand
    ...
    .action((options) => {
        ...
        try {
            execSync(`docker ps --format '{{.Names}}' | grep -q "^${options.container}$"`, { stdio: 'inherit' });
        } catch (error) {
        ...
        const backupFile = path.join(backupDir, `${dbName}_${timestamp}.${options.format === 'custom' ? 'dump' : 'sql'}`);
        ...
        try {
            execSync(`docker exec -e PGPASSWORD=${dbPassword} ${options.container} pg_dump -U ${dbUser} -h localhost ${formatFlag} ${dbName} > ${backupFile}`, { stdio: 'inherit' });
