...
export function calculatePaceVariability(velocityData: number[]): number {
  ...
  const variance = movingVelocities.reduce((sum, v) => sum + Math.pow(v - mean, 2), 0) / movingVelocities.length
...
export function calculatePaceZones(
  ...
  timeInZones.forEach(zone => {
    zone.percentage = totalMovingTime > 0 
      ? Math.round((zone.timeInZone / totalMovingTime) * 100) 
      : 0
  })
 ...
 */
export function calculateAveragePace(totalTimeSeconds: number, totalDistanceMeters: number): number {
...
export function analyzePacingStrategy(lapSplits: any[]) {
  ...
  const firstHalfAvgPace = firstHalfLaps.reduce((sum, lap) => sum + lap.paceSeconds, 0) / firstHalfLaps.length
  const secondHalfAvgPace = secondHalfLaps.reduce((sum, lap) => sum + lap.paceSeconds, 0) / secondHalfLaps.length
