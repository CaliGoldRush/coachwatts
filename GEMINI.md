...
By following these guidelines, you can avoid common pitfalls and ensure that modals are implemented correctly and consistently across the application. - [Nuxt Server Patterns](./.roo/rules-code/nuxt-server-patterns.md) - [Repository Pattern](./.roo/rules-code/repository-pattern.md) - [Server Management](./.roo/rules-code/server-management.md) - [TypeScript Guidelines](./.roo/rules-code/typescript-guidelines.md)
...
- When debugging or implementing Nuxt UI components, ALWAYS use the available MCP tools (`list-documentation-pages`, `get-documentation-page`) to check the official documentation first. Common pitfalls (like `model-value` vs `value` for `UProgress`, or complex slot/v-model logic for `UModal`) can be avoided by verifying the API reference. We have historically struggled with Modal implementations, so extra care is required there.
