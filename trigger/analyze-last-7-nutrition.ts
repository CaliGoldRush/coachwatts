    ...
    sections: {
      ...
      items: {
        ...
        properties: {
          title: {
            type: "string",
            description: "Section title (e.g., Caloric Balance, Macronutrient Distribution, Weekly Patterns)"
          },
          status: {
            type: "string",
...
function convertStructuredToMarkdown(analysis: any): string {
  ...
  if (analysis.nutrition_summary) {
    ...
    if (metrics.consistency_score) markdown += `- **Consistency Score**: ${metrics.consistency_score.toFixed(0)}%\n`;
  }
