    ...
    if (adjustments.durationMinutes || adjustments.intensity) {
      await prisma.plannedWorkout.update({
        ...
        data: {
          ...
          workIntensity: adjustments.intensity ? getIntensityScore(adjustments.intensity) : undefined,
          // If title/description changed, update them too? Assuming title stays.
        }
      });
      // Refresh local var
      workout.durationSec = adjustments.durationMinutes ? adjustments.durationMinutes * 60 : workout.durationSec;
      workout.workIntensity = adjustments.intensity ? getIntensityScore(adjustments.intensity) : workout.workIntensity;
    }
