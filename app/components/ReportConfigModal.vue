  ...
  <div class="p-6 space-y-6">
    ...
    <div>
        ...
        <div v-else-if="config.timeframeType === 'count'">
          ...
          <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">
            Analyze the last {{ config.count || 0 }} {{ config.dataType === 'workouts' ? 'workouts' : 'nutrition days' }}
          </p>
        </div>
...
const dataTypeOptions = [
  { value: 'workouts', label: 'Workouts Only', icon: 'i-heroicons-chart-bar', description: 'Analyze training sessions and performance' },
  { value: 'nutrition', label: 'Nutrition Only', icon: 'i-heroicons-cake', description: 'Analyze dietary intake and nutrition patterns' },
  { value: 'both', label: 'Workouts & Nutrition', icon: 'i-heroicons-squares-2x2', description: 'Comprehensive analysis of training and nutrition' }
]
...
const focusAreaOptions = [
  { value: 'general', label: 'General Analysis', icon: 'i-heroicons-document-text', description: 'Comprehensive overview and insights' },
  { value: 'performance', label: 'Performance Trends', icon: 'i-heroicons-arrow-trending-up', description: 'Focus on progress and performance metrics' },
  { value: 'recovery', label: 'Recovery & Fatigue', icon: 'i-heroicons-heart', description: 'Analyze recovery patterns and training load' },
  { value: 'consistency', label: 'Training Consistency', icon: 'i-heroicons-calendar', description: 'Evaluate adherence and consistency' },
  { value: 'nutrition', label: 'Nutrition Quality', icon: 'i-heroicons-beaker', description: 'Deep dive into dietary patterns' }
]
...
const isConfigValid = computed(() => {
  if (config.value.timeframeType === 'days') {
    ...
    return config.value.startDate && config.value.endDate && config.value.startDate <= config.value.endDate
  }
  return false
})
