<template>
  <UCard>
    ...
    <div class="flex flex-col flex-1 w-full">
      ...
      <!-- Pagination -->
      <div v-if="data?.items && data.items.length > 0" class="flex items-center justify-between px-4 py-3.5 border-t border-accented text-sm">
        <div class="text-muted">
          Showing {{ data.pagination.from }} to {{ data.pagination.to }} of {{ data.pagination.total }} records
        </div>
...
const { data, status, refresh: refreshData } = await useFetch('/api/analytics/llm-usage/history', {
  query: computed(() => ({
...
const columns: TableColumn<UsageItem>[] = [{
  ...
  cell: ({ row }) => {
    ...
    return h('div', { class: 'text-right text-muted' }, `${((duration ?? 0) / 1000).toFixed(2)}s`)
  }
}, {
  accessorKey: 'success',
