<template>
  <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
    ...
    <div class="mb-6 bg-gray-50 dark:bg-gray-700/50 rounded-lg p-4 flex flex-wrap items-center gap-6 border border-gray-100 dark:border-gray-700">
      <!-- Adherence Score (if exists) -->
      <div v-if="adherence" class="flex items-center gap-3 pr-6 border-r border-gray-200 dark:border-gray-600">
        <div class="relative w-14 h-14 flex items-center justify-center rounded-full border-4 flex-shrink-0" :class="getScoreBorderColor(adherence.overallScore)">
        ...
        <div class="hidden sm:block">
          ...
          <div class="text-sm font-semibold text-gray-900 dark:text-white leading-tight">{{ getScoreLabel(adherence.overallScore) }}</div>
        </div>
      </div>

      <!-- Planned Workout Details -->
      <div v-if="plannedWorkout" class="flex-1 min-w-[200px] flex flex-wrap items-center gap-x-6 gap-y-2">
        <div class="flex-1 min-w-[150px]">
          <div class="text-[10px] text-gray-500 uppercase font-bold tracking-wider mb-0.5">Target Plan</div>
          <div class="font-bold text-gray-900 dark:text-white truncate">{{ plannedWorkout.title }}</div>
        </div>
        
        <div class="flex items-center gap-3 text-xs">
          <div v-if="plannedWorkout.type" class="flex items-center gap-1 px-2 py-0.5 rounded-md bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 shadow-sm">
            <UIcon name="i-heroicons-tag" class="w-3.5 h-3.5 text-gray-400" />
            <span class="font-medium text-gray-700 dark:text-gray-300">{{ plannedWorkout.type }}</span>
          </div>
          
          <div v-if="plannedWorkout.durationSec" class="flex items-center gap-1 px-2 py-0.5 rounded-md bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 shadow-sm">
            <UIcon name="i-heroicons-clock" class="w-3.5 h-3.5 text-gray-400" />
            <span class="font-medium text-gray-700 dark:text-gray-300">{{ formatDuration(plannedWorkout.durationSec) }}</span>
          </div>
        </div>
      </div>
    </div>
    ...
    <div v-if="adherence" class="space-y-6">
      ...
      <div v-if="adherence.deviations && adherence.deviations.length > 0">
        <h3 class="text-sm font-semibold text-gray-900 dark:text-white mb-3 uppercase tracking-wide">Key Deviations</h3>
        <div class="space-y-3">
          <div 
          ...
          >
            <div class="flex justify-between items-start">
              <span class="font-medium text-sm text-gray-900 dark:text-white">{{ dev.metric }}</span>
      ...
      <div v-if="adherence.recommendations && adherence.recommendations.length > 0">
        <h3 class="text-sm font-semibold text-gray-900 dark:text-white mb-3 uppercase tracking-wide">Coach Recommendations</h3>
        <ul class="space-y-2">
          <li 
          ...
          >
            <UIcon name="i-heroicons-light-bulb" class="w-4 h-4 text-yellow-500 mt-0.5 flex-shrink-0" />
            <span>{{ rec }}</span>
          </li>
        </ul>
      </div>
