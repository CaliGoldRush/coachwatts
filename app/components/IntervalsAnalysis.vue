  ...
  <div class="space-y-6">
    ...
    <div v-else-if="data" class="space-y-6">
      ...
      <div v-if="chartConfig" class="bg-white dark:bg-gray-800 rounded-lg shadow p-4 h-80">
        <h3 class="text-sm font-bold text-gray-500 dark:text-gray-400 mb-2 uppercase tracking-wider">Interval Visualization</h3>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div v-if="bestEffort" class="bg-white dark:bg-gray-800 rounded-lg shadow p-4 border-l-4 border-amber-500">
        ...
        <!-- HR Recovery -->
        <div v-if="data.recovery" class="bg-white dark:bg-gray-800 rounded-lg shadow p-4 border-l-4 border-green-500">
      ...
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow overflow-hidden">
        ...
        >
          ...
          <UBadge :color="data.detectionMetric === 'power' ? 'primary' : data.detectionMetric === 'pace' ? 'info' : 'error'" variant="subtle">
      ...
      <div v-if="peakEfforts.length > 0" class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
          ...
          >
            <div class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase">{{ peak.duration_label }}</div>
        ...
        tooltip: {
          callbacks: {
            ...
            afterBody: function(tooltipItems: any[]) {
              ...
              const interval = data.value.intervals.find((i: any) =>
                i.start_time <= time && i.end_time >= time && i.type === 'WORK'
              )
...
const bestEffort = computed(() => {
  ...
  const peaks = data.value.peaks.power.length > 0 ? data.value.peaks.power :
                data.value.peaks.pace.length > 0 ? data.value.peaks.pace : 
...
const peakEfforts = computed(() => {
  ...
  return data.value.peaks.power.length > 0 ? data.value.peaks.power :
         data.value.peaks.pace.length > 0 ? data.value.peaks.pace : 
         data.value.peaks.heartrate
})
