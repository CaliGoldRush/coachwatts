...
const hasError = computed(() => {
  return props.toolCall.response && typeof props.toolCall.response === 'object' && 'error' in props.toolCall.response
})
...
<template>
  <div class="my-3 rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/50 overflow-hidden">
    <!-- Tool Call Header -->
    <button
    ...
    >
      <div class="flex items-center gap-3">
        ...
        <div class="flex flex-col items-start">
          <span class="font-medium text-sm" :class="hasError ? 'text-red-600 dark:text-red-400' : 'text-gray-900 dark:text-gray-100'">
    ...
    <div v-if="isExpanded" class="p-4 space-y-4">
      ...
      <div v-if="Object.keys(toolCall.args).length > 0">
        <h4 class="text-xs font-semibold text-gray-700 dark:text-gray-300 mb-2 flex items-center gap-2">
          <UIcon name="i-heroicons-arrow-right-circle" class="w-4 h-4" />
          Input Parameters
        </h4>
        <div class="bg-white dark:bg-gray-950 rounded border border-gray-200 dark:border-gray-800 p-3">
          <pre class="text-xs text-gray-600 dark:text-gray-400 overflow-x-auto"><code>{{ formatJson(toolCall.args) }}</code></pre>
        </div>
      </div>
      ...
      <div>
        ...
        >
          <pre 
            ...
            :class="hasError ? 'text-red-700 dark:text-red-400' : 'text-gray-600 dark:text-gray-400'"
