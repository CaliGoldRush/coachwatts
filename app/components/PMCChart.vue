  ...
  <div class="pmc-chart">
    ...
    <div v-else-if="pmcData">
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <div class="bg-gradient-to-br from-purple-50 to-purple-100 dark:from-purple-900/20 dark:to-purple-800/30 rounded-lg p-4 shadow">
          <div class="flex items-center justify-between mb-2">
            <span class="text-xs text-purple-600 dark:text-purple-400 font-semibold">CTL (Fitness)</span>
        ...
        <div class="bg-gradient-to-br from-yellow-50 to-yellow-100 dark:from-yellow-900/20 dark:to-yellow-800/30 rounded-lg p-4 shadow">
          <div class="flex items-center justify-between mb-2">
            <span class="text-xs text-yellow-600 dark:text-yellow-400 font-semibold">ATL (Fatigue)</span>
      ...
      <div class="mt-4 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-800">
        <h4 class="text-sm font-semibold text-blue-900 dark:text-blue-100 mb-2">Understanding PMC</h4>
        <ul class="text-xs text-blue-800 dark:text-blue-200 space-y-1">
          <li><strong>CTL (Fitness):</strong> Long-term training load (~42 days). Higher = more fitness.</li>
          <li><strong>ATL (Fatigue):</strong> Short-term training load (~7 days). Higher = more fatigue.</li>
          <li><strong>TSB (Form):</strong> CTL - ATL. Positive = fresh, negative = fatigued, -10 to +5 = optimal racing form.</li>
        </ul>
      </div>
    </div>
  </div>
...
const chartData = computed(() => {
  ...
  const labels = data.map((d: any) => new Date(d.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }))
