    ...
    <UCard>
      ...
      <div class="space-y-4">
        <p class="text-sm text-neutral-500">
          To connect a coach, generate an invite code and share it with them. They can then enter this code in their dashboard to connect.
        </p>

        <div v-if="activeInvite && activeInvite.status === 'PENDING'" class="bg-primary-50 dark:bg-primary-900/20 p-6 rounded-lg text-center border border-primary-100 dark:border-primary-800">
          <p class="text-xs uppercase tracking-widest text-primary-600 dark:text-primary-400 font-bold mb-2">Your Invite Code</p>
          <div class="text-4xl font-mono font-bold tracking-widest mb-2">{{ activeInvite.code }}</div>
          <p class="text-xs text-neutral-500">Expires on {{ formatDate(activeInvite.expiresAt) }}</p>
...
async function removeCoach() {
  ...
  try {
    ...
    await $fetch(`/api/coaching/coaches/${selectedRelationship.value.coachId}`, { method: 'DELETE' })
