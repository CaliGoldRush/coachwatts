import { detectIntervals, findPeakEfforts, calculateHeartRateRecovery } from '../server/utils/interval-detection'

// Mock Data Generators
function generateSineWave(duration: number, period: number, amplitude: number, offset: number): number[] {
  const data: number[] = []
...
function generateIntervals(duration: number): { time: number[], power: number[], hr: number[] } {
  ...
  for (let i = 0; i < duration; i++) {
    ...
    const cycle = i % 600
    if (cycle < 300 && i > 600) { // Work interval (starts after 10m warmup)
...
async function runTests() {
  ...
  workIntervals.forEach((interval, idx) => {
    console.log(`  Interval ${idx + 1}: ${interval.duration}s @ ~${Math.round(interval.avg_power!)}W`)
  })
  
  // Test 2: Peak Efforts
  console.log('\nTest 2: Peak Efforts')
  const peaks = findPeakEfforts(data.time, data.power, 'power')
  console.log('Found peaks:', peaks.map(p => `${p.duration_label}: ${p.value}W`))
  ...
  if (recovery) {
    console.log(`Peak HR: ${recovery.peakHr}, Recovery HR (1min): ${recovery.recoveryHr}, Drop: ${recovery.drops}`)
  } else {
